define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        ...
        forget: function () {
            //异步验证图形二维码是否正确 正确解除发送短信button的disabled
            $('#captcha').blur(function(){
                $.ajax({
                    url: 'ajax/checkCaptcha',
                    dataType: 'json',
                    data: {captcha: $("input[name='captcha']").val(),username:$("input[name='username']").val()},
                    method:'POST',
                    success: function (res) {
                        ...
                        if(res.code===1){
                            $('#pushCaptcha').removeAttr("disabled");
                        }
                    }, error: function () {
                        Toastr.error(__('Network error'));
                    }
            ...
            var countdown=60;
            $('#pushCaptcha').click(function(){
                $.ajax({
                    url: 'ajax/pushCaptcha',
                    dataType: 'json',
