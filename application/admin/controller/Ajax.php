...
class Ajax extends Backend
{
    ...
    protected $noNeedLogin = ['lang','checkCaptcha','pushCaptcha'];
...
//        $rule['username']  = 'require';
        ...
        $validate = new Validate($rule, [], ['username' => __('Username'), 'captcha' => __('验证码')]);
     ...
     * 忘记密码异步请求短信验证码
     * */
    public function pushCaptcha()
    {
        $mobile = $this->request->post('phone');
        $lasttime = DB::name('sms')->where(['mobile'=>$mobile])->order('id desc')->limit(1)->value('createtime');
        if(time()-$lasttime<60){
            $this->error('请求频繁');
        }
        $code = rand(100000,999999);
        $content = "【食迅生鲜】"."您正在使用重置密码功能,验证码为:" . $code;
//        $url = "https://dx.ipyy.net/sms.aspx";
//        $account = "AA00796";
//        $password = "AA0079645";
        $extno = "";
        $sendtime = "";
        $result = $this->sendMO($mobile,$extno,$content,$sendtime);

        $xml = simplexml_load_string($result);
        $jsonStr = json_encode($xml);
        $jsonArray = json_decode($jsonStr,true);
        if($jsonArray['returnstatus'] == 'Faild'){
            $this->error($jsonArray['message']);
        }else{
            $insert = [
                'event' => '忘记密码',
                'mobile' => $mobile,
    ...
    //发送短信
    public function sendMO($mobiles,$extno,$content,$sendtime){
        $body=array(
            'action'=>'send',
            'userid'=>'',
            'account'=>'XPT30204',
